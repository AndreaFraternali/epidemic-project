...
{
  par.legS = {"Suscettibili", font_, 24};
  par.legS.setFillColor(sf::Color::Black);
  par.legS.setPosition(.8 * display_width, .02 * display_height);
  par.legI = {"Infetti", font_, 24};
  par.legI.setFillColor(sf::Color::Black);
  par.legI.setPosition(.8 * display_width, .06 * display_height);
  par.legR = {"Rimossi", font_, 24};
  par.legR.setFillColor(sf::Color::Black);
  par.legR.setPosition(.8 * display_width, .1 * display_height);

  par.Scirc = sf::CircleShape{8};
  par.Scirc.setFillColor(sf::Color::Green);
  par.Scirc.setPosition(.95 * display_width, .035 * display_height);
  par.Icirc = sf::CircleShape{8};
  par.Icirc.setFillColor(sf::Color::Red);
  par.Icirc.setPosition(.95 * display_width, .075 * display_height);
  par.Rcirc = sf::CircleShape{8};
